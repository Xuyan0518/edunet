...
//         setStudents(data);
//         // if current selected student is invalid, clear it
//         if (selectedStudent && !data.some((s: any) => s.id === selectedStudent)) {
//           setSelectedStudent('');
//         }
//       } catch (err) {
...
//         const url = `${buildApiUrl('feedback/one')}?studentId=${encodeURIComponent(selectedStudent)}&weekStarting=${encodeURIComponent(startStr)}`;
//         const res = await fetch(url, { signal: ac.signal });

//         if (ac.signal.aborted) return;

//         if (!res.ok) {
//           // backend returns 200 with null on not found per your code,
//           // but keep a guard in case of non-200
//           throw new Error('Failed to fetch weekly feedback');
//         }
...
//           setAreasToImprove(Array.isArray(data.areasToImprove) && data.areasToImprove.length ? data.areasToImprove : ['']);
...
//     run();
//     return () => ac.abort();
//   }, [selectedStudent, weekStarting, toast]);
...
//     if (date && date.getDay() === 0) setWeekStarting(date);
//   };
...
//       toast({ title: 'Error', description: 'Please fill out all strengths/areas or remove empty ones', variant: 'destructive' });
//       return false;
//     }
//     return true;
//   };
...
//           body: JSON.stringify({ ...payload, id: existingId }), // backend accepts id in body
//         });
//       } else if (!existingId) {
//         // create
...
//                   // Let the fetch effect determine existing vs new. Avoid pre-toggling isEditing here.
//                 }}
//                 disabled={isLoading}
//               >
...
//                 size="sm"
//                 onClick={() => {
//                   // backup current values so Cancel can restore
//                   setBackup({
