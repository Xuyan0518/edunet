...
6) 若某科没有可用分数，只评价学习投入、完成情况和问题点，不要硬写分数趋势。
7) 如 analytics.englishAnalytics.hasEnglishData=true，必须输出 englishSpecialAnalysis，且只依据 englishAnalytics。
8) priority 只能是 high/medium/low。
9) dataQualityNotesForTeacher 需要参考 analytics.aiGuidance.dataQualityNotes，但不要在家长可读主文案反复出现“数据有限”等机械句式。
10) 输出必须是单个 valid JSON object，不要 markdown 代码块，不要 JSON 外文字。
